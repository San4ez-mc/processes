...
const SCENARIO_SALES = 'sales_process'
const SCENARIO_OPERATIONS = 'operations_process'
const SCENARIO_FINANCE = 'finance_process'

const SCENARIO_HINTS = {
  [SCENARIO_MAIN]: 'Сценарій: Основний бізнес-процес (повний шлях від ліда до оплати).',
  [SCENARIO_SALES]: 'Сценарій: Продаж і кваліфікація (фокус на від заявки до згоди клієнта).',
  [SCENARIO_OPERATIONS]: 'Сценарій: Виконання і виробництво (фокус на операційній частині).',
  [SCENARIO_FINANCE]: 'Сценарій: Оплата і закриття (фокус на рахунках, оплаті, актах).',
}
...
async function launchScenario(userId, payload) {
  const scenario = [SCENARIO_MAIN, SCENARIO_SALES, SCENARIO_OPERATIONS, SCENARIO_FINANCE].includes(payload)
    ? payload
    : SCENARIO_MAIN
  ...
  const session = await db.getOrCreateSession(userId)
  if (scenario === SCENARIO_SALES) {
    session.current_block = 2
    session.completed_blocks = [0, 1]
    await db.saveSession(session)
    await safeSendMessage(
      userId,
      `${SCENARIO_HINTS[SCENARIO_SALES]}\n\nПочнемо одразу з продажу. Що відбувається після отримання заявки? Хто і як зв'язується з клієнтом?`
    )
    return
  }

  if (scenario === SCENARIO_OPERATIONS) {
    session.current_block = 3
    session.completed_blocks = [0, 1, 2]
    await db.saveSession(session)
    await safeSendMessage(
      userId,
      `${SCENARIO_HINTS[SCENARIO_OPERATIONS]}\n\nПочнемо з операційної частини. Після погодження клієнта що відбувається далі? Хто запускає роботу?`
    )
    return
  }

  session.current_block = 4
  session.completed_blocks = [0, 1, 2, 3]
  await db.saveSession(session)
  await safeSendMessage(
    userId,
    `${SCENARIO_HINTS[SCENARIO_FINANCE]}\n\nПочнемо з оплат. Як і коли клієнт платить: передоплата, постоплата чи частинами?`
  )
}
